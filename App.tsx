...
import App12PosterDesigner from './app12/App';
...
const Launcher: React.FC<{ onSelect: (view: 'pro' | 'batch' | 'poster' | 'ecom' | 'refine' | 'lumi' | 'presets' | 'correct' | 'station' | 'director' | 'cny' | 'poster-designer') => void }> = ({ onSelect }) => {
  ...
  const handleCardClick = (type: 'pro' | 'batch' | 'poster' | 'ecom' | 'refine' | 'lumi' | 'presets' | 'correct' | 'station' | 'director' | 'cny' | 'poster-designer') => {
    ...
    <div className="min-h-screen bg-[#020617] flex flex-col relative overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto w-full z-10 px-6 py-24 md:py-32 space-y-24 animate-in fade-in zoom-in-95 duration-1000 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <button onClick={() => handleCardClick('poster-designer')} className="group relative flex flex-col p-10 rounded-[48px] bg-slate-900/40 border border-slate-800 hover:border-orange-500 transition-all duration-700 backdrop-blur-xl hover:-translate-y-4 overflow-hidden text-left">
            <div className="absolute top-0 right-0 w-64 h-64 bg-orange-600/10 blur-[80px] -mr-32 -mt-32"></div>
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center border bg-orange-500/10 border-orange-500/20 text-orange-400 mb-8 shadow-xl"><ShoppingBag className="w-8 h-8" /></div>
            <h2 className="text-2xl font-black text-white tracking-tight mb-4">AI 智能海报设计师</h2>
            <p className="text-slate-400 text-sm leading-relaxed mb-8">全自动商业海报生成。深度整合营销策略，一键生成极具视觉冲击力的品牌海报。</p>
            <div className="mt-auto flex items-center gap-2 font-black text-xs uppercase tracking-widest text-orange-400 group-hover:gap-4 transition-all">立即设计 <ArrowRight className="w-4 h-4" /></div>
          </button>
...
const App: React.FC = () => {
  const [view, setView] = useState<'launcher' | 'pro' | 'batch' | 'poster' | 'ecom' | 'refine' | 'lumi' | 'presets' | 'correct' | 'station' | 'director' | 'cny' | 'poster-designer'>('launcher');
  const [stationPrefill, setStationPrefill] = useState<{ prompt: string; negative?: string; images: string[] } | null>(null);

  const handleUsePreset = (data: { prompt: string; negative?: string; images: string[] }) => {
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'poster-designer': return <App12PosterDesigner />;
