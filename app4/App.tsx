...
import { 
  ...
  Target, Zap, ShieldCheck, ShoppingBag, Radio, MessageSquare, Send, Trash2
} from 'lucide-react';
                  ...
                  {activeAnalysisTab === 'analysis' ? (
                    <div className="space-y-8 pb-12">
                       ...
                       <section className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm space-y-8">
                          ...
                          <div className="grid md:grid-cols-3 gap-6">
                             <div className="space-y-4">
                                <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">5大产品卖点</h5>
                             ...
                             <div className="space-y-4">
                                <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">5大消费者买点</h5>
                                <div className="space-y-1.5">
                       ...
                       <section className="bg-slate-900 p-10 rounded-[3rem] text-white space-y-8 shadow-2xl">
                          ...
                          <div className="space-y-6">
                             <div className="space-y-4">
                                <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] border-l-4 border-indigo-600 pl-3">内容选题与脚本库</h4>
              ...
              <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-6 sticky top-24 space-y-6">
                ...
                <div className="space-y-3">
                   <div className="flex items-center justify-between px-1">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">核心参考图矩阵 ({referenceImages.length}/3)</label>
                     {referenceImages.length > 0 && <button onClick={()=>setReferenceImages([])} className="text-[9px] text-indigo-600 font-bold hover:underline">全部清空</button>}
                   </div>
                   
                   <div className="grid grid-cols-3 gap-2">
                     {referenceImages.map((u, i) => (
                       <div key={i} className="relative aspect-square rounded-xl overflow-hidden border-2 border-indigo-100 shadow-sm group">
                         <img src={u} className="w-full h-full object-cover" />
                         <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button onClick={(e) => { e.stopPropagation(); setReferenceImages(prev => prev.filter((_, idx) => idx !== i)); }} className="p-1.5 bg-rose-500 text-white rounded-full hover:scale-110 transition-transform">
                              <Trash2 className="w-3 h-3" />
                            </button>
                         </div>
                         <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-indigo-600 text-[7px] text-white font-black uppercase rounded shadow-sm">
                            {i === 0 ? 'Primary' : 'Style'}
                         </div>
                       </div>
                     ))}
                     ...
                     {referenceImages.length < 3 && (
                       <div 
                        ...
                        className="aspect-square border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 hover:bg-white transition-all group"
                       >
                         <Plus className="w-5 h-5 text-slate-300 group-hover:text-indigo-400 group-hover:scale-110 transition-all" />
                         <span className="text-[8px] font-black text-slate-400 uppercase mt-1 tracking-tighter">
                            {referenceImages.length === 0 ? '添加主图' : '添加风格'}
                         </span>
                       </div>
                     )}
                   </div>
