...
5. 提示词必须符合以下模板格式：
这是一张写实风格的电商主图。画面主体是参考图中产品的特写画面，有较强的平面设计风格。[具体场景描述，必须出现产品包装，且产品包装占主导地位，突出产品属性和名称，描述产品时可描述为如参考图中的绿色盒子的乌梅三豆膏]。【文案设计】：1.左上角文案：在左上角空白处使用[字体及字体设计，如"极大的渐变加描边加粗宋体书写"][产品核心卖卖点方案，不大于10个汉字，如洗洗肺 更清爽]，[当主标题文案字数大于9个汉字可换行，小于8个汉字时不换行，如不换行时的描述"文字不换行"]，下方紧跟稍小的[副标题字体及字体设计，如"薄荷绿文字"][副标题文案如排出浊气 呼吸顺及宣肺利咽]。3.底部文案：画面最下方设计一条贯穿左右的[色块的颜色设计，如"清透蓝渐变"]波浪形色块(Banner)，色块内左侧是一个圆形图标，图标内用[图标内的字体名称，如超大白色粗黑体][图标内的文如：通透]，右侧用[促销或信任背书文字及字体设计，如：字体名称：白色圆体，文案：桔梗桑叶 清肺热]。 raw，写实，8k，构图饱满，无分割线。
