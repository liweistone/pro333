...
async function startServer() {
  ...
  app.post("/api/v1/chat/completions", async (req, res) => {
    try {
      const response = await fetch("https://api.aishuch.com/v1/chat/completions", {
        method: "POST",
        headers: {
  ...
  app.post("/api/v1/images/generations", async (req, res) => {
    try {
      const response = await fetch("https://api.aishuch.com/v1/images/generations", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${AUTH_KEY}`
  ...
  app.get("/api/v1/tasks/:taskId", async (req, res) => {
    try {
      ...
      const response = await fetch(`https://api.aishuch.com/v1/tasks/${taskId}?language=${language}`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${AUTH_KEY}`
        }
